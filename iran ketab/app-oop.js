...
const searchBoxContainerHeader = document.querySelector(
...
const recommendedPackagesDiv = document.querySelector(
  "#Recommended-Packages-right",
);
...
const fetchBook = new fetchAndShow("/api/new-books");
fetchBook
  .fetchAndShowBook()
  .then(() => {
...
class creatLastVisitItem {
  ...
  createLastVisitedItemAdd(element) {
    if (this.e.target.classList.contains("buttom-cart")) {
      ...
      const existingImage = this.lastVisitContiner.querySelectorAll(
        ".last-visit-item-image img",
      );
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("overly-search-show-header")) {
    //شرط اینپوت هدر گداشته شد
    body.classList.remove("no-scroll");
    overlySearchHeader.classList.remove("overly-search-show-header");
    searchBoxContainerHeader.classList.remove(
      "search-box-container-header-show",
    );
...
bookRightFlash.forEach((flash) => {
  flash.addEventListener("click", (e) => {
    ...
    let selectBookCenter = selectBookProductGrPa.querySelector(
      ".book-products-center",
    );
...
bookProducts.forEach((bookCenter) => {
  bookCenter.addEventListener("scroll", () => {
    ...
    if (scrollLeftBook + clientWidthBook >= scrollWidthBook - tolerance) {
      //به راست رسید
...
overlySearchHeader.addEventListener("click", (e) => {
  //روی اینپوت هدر تعریف شد
  if (e.target.classList.contains("search-input")) {
    e.stopPropagation();
