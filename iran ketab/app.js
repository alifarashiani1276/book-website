...
const books = async ()=>{
   try{
     const response = await fetch(fetchLibrary)
    if(!response.ok) throw new Error(`خطای HTTP! وضعیت: ${response.status}`);

    const data = await response.json()

    bookProducts.forEach((bookProduct)=>{
    ...
    bookProducts.forEach((book)=>{ //پر کردن ارایه cartsArr
      let h2Cart = book.querySelectorAll(".cart h2")
      let pCart = book.querySelectorAll(".cart p")
...
function createLastVisitedItem(e){
   if(e.target.classList.contains("buttom-cart")){
    
    let cartElm = e.target.closest(".cart")
    if(!cartElm){
    ...
    const existingImage = lastVisitContiner.querySelectorAll(".last-visit-item-image img")
    ...
    if(lastVisitContiner){
         lastVisitContiner.appendChild(lastVisitDiv)//اضافه به دام
    }
  }
}

document.addEventListener("click",(e)=>{
   createLastVisitedItem(e);
...
function filterbook(textInputsearch,ArrBooksText,result){
  result.textContent=""
  let bookFiltered = ArrBooksText.filter((book)=>{
     return book.textContent.includes(textInputsearch)
  })

  bookFiltered.forEach((book)=>{
    let cart = book.closest(".cart") 
...
function handleSearch(inputElement, resultElement) {
  inputElement.addEventListener("input", (e) => {
    ...
    if (inputValue.length > 0) {
      ...
      words.forEach((word) => {
        let wordChar = word.charAt(0).toUpperCase();
        let wordLowerCase = word.slice(1).toLowerCase();
        let restText = (wordChar + wordLowerCase).trim();

        filterbook(restText, cartsArr, resultElement);
      });
    }
  });
}
